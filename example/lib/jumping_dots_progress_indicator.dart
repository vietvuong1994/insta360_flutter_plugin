...
class _JumpingDot extends AnimatedWidget {
  ...
  const _JumpingDot({Key? key, required Animation<double> animation, this.color, this.fontSize}) : super(key: key, listenable: animation);
...
class _JumpingDotsProgressIndicatorState extends State<JumpingDotsProgressIndicator> with TickerProviderStateMixin {
  ...
  void _addListOfDots(int index) {
    _widgets.add(
      Padding(
        padding: EdgeInsets.only(right: dotSpacing! / 2, left: dotSpacing! / 2),
        child: _JumpingDot(
